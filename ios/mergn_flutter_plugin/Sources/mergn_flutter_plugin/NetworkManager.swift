...
class NetworkManager {
   ...
   // private let baseURL = "https://devapi.mergn.com/sdk-management/api/"  // Replace with your API base URL
    ...
    private func createHeaders() -> [String: String] {
        //let defaults = UserDefaults(suiteName: "com.mergn.alphala")
        //let apiKey = defaults?.string(forKey: "api_mergn_key")
        func getMergnApiKey() -> String {

            if let appGroupId = Bundle.main.object(forInfoDictionaryKey: "APP_GROUP_ID_MERGN") as? String {
                if let defaults = UserDefaults(suiteName: appGroupId) {
                    let apiKey = defaults.string(forKey: "api_mergn_key") ?? "default_api_key"
                    return apiKey
                } else {
            ...
            //Handling in case Group Id Not defined
            return UserDefaults.standard.string(forKey: EventManager.shared.apiMergnKey) ?? "default_api_key"
        }
        return [
            ...
            "authorization": getMergnApiKey(),  // Replace with actual token // Hard corded
