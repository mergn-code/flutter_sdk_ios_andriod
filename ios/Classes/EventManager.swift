...
public class EventManager {
    ...
    private let mergnNotificationView  = "mergn_notfication_view"
    private let mergnNotificationClick  = "mergn_notfication_click"
    private let eventMapMergn  = "event_map_mergn"
    ...
    //Do call app launch here
        do {
            ...
            if let appGroupId = Bundle.main.object(forInfoDictionaryKey: "APP_GROUP_ID_MERGN") as? String {
                // Ensure you have access to the correct UserDefaults suite (App Group)
                 if let defaults = UserDefaults(suiteName: appGroupId) {
                     // Save the API key to UserDefaults
                     defaults.set(apiKey, forKey: "api_mergn_key")

                     // Optionally, synchronize the changes (usually not necessary, but useful in some cases)
                     defaults.synchronize()
                 } else {
                     print("Failed to access UserDefaults for App Group.")
                 }
            }
            else {print("APP_GROUP_ID_MERGN not found")}
            ...
            DispatchQueue.global().asyncAfter(deadline: .now() + 6.0) {
                do {
                    // First, add app installed event
                    try self.addAppInstalledEvent()

                    // Then, add app launch event
                    try self.addAppLaunchEvent()

                } catch {
                    print("Error posting app launch and app install to server: \(error.localizedDescription)")
                }
            }
    ...
    public func saveCustomerId(customerId : String) -> String {
       let saveCustomerId = customerId
        ...
        if let appGroupId = Bundle.main.object(forInfoDictionaryKey: "APP_GROUP_ID_MERGN") as? String {
            if let defaults = UserDefaults(suiteName: appGroupId) {
                // Save the API key to UserDefaults
                defaults.set(customerId, forKey: customerMergnKey)
    ...
    public func getCustomerId() -> String {
        ...
        if let appGroupId = Bundle.main.object(forInfoDictionaryKey: "APP_GROUP_ID_MERGN") as? String {
            if let defaults = UserDefaults(suiteName: appGroupId) {
                let customerId = defaults.string(forKey: customerMergnKey) ?? ""
                return customerId
            } else {
                return ""
            }
            ...
            NetworkManager.shared.getEventList { result in
                ...
                case .success(let eventList):
                    ...
                    if let appGroupId = Bundle.main.object(forInfoDictionaryKey: "APP_GROUP_ID_MERGN") as? String {
                        if let defaults = UserDefaults(suiteName: appGroupId){
                            do {
                                let encoded = try JSONEncoder().encode(self.eventMap)
                                defaults.set(encoded, forKey: self.eventMapMergn)
                            } catch {
                                print("Failed to encode eventMap: \(error)")
                            }
                        }
                    }
                    else {print("APP_GROUP_ID_MERGN not found")}
...
public func getAttributeValue(attributeId : String) -> String {
        guard let attrbiuteValue = UserDefaults.standard.string(forKey: attributeId), !attrbiuteValue.isEmpty else {
            print("Adding Attribute value for the first time !!")
            return ""
        }
        ...
        do {
            if let appGroupId = Bundle.main.object(forInfoDictionaryKey: "APP_GROUP_ID_MERGN") as? String {
               ...
               // defaults?.set("value", forKey: "key")
                print(appGroupId + " Group id value")
            }
            else {print("APP_GROUP_ID_MERGN not found")}
            if eventMap.isEmpty {
                getEventList()
            ...
            if eventMap.keys.contains(eventName) {
                ...
                if let propertiesEvent = eventDetails?.eventProperty {
                    ...
                    if propertiesEvent.keys.contains("sdk-version") {
                        var newEventProperty2 = EventRequestModel.EventProperty(eventPropertyId: propertiesEvent["sdk-version"]?.id ?? 0, value: "4")
                        eventProperties.append(newEventProperty2)
                    }
                }

                print("Event Propertie are \(eventProperties)")
    ...
    public func sendPopupEvent(eventName: String, popupActionProperty: String,
        ...
        do {
            //putting the followiung condition incase of events are being called out of the app.
            //i.e. notificaiton events
            if(eventName == EventNames.notificationViewed.rawValue ||
               eventName == EventNames.notificationClicked.rawValue){

                // Ensure you have access to the correct UserDefaults suite (App Group)
                if let appGroupId = Bundle.main.object(forInfoDictionaryKey: "APP_GROUP_ID_MERGN") as? String {
                    //Adding condition for notification viewed and notification clicked
                    if let defaults = UserDefaults(suiteName: appGroupId){
                        if let data = defaults.data(forKey: eventMapMergn) {
                            do {
                                let decoded = try JSONDecoder().decode([String: Event].self, from: data)
                                eventMap = decoded
                                //print("Events are \(eventMap)")
                                print("Events are already available")
                            } catch {
                                print("Failed to decode eventMap: \(error)")
                                eventMap = [:]
                                //print("Events are \(eventMap)")
            ...
            if eventMap.keys.contains(eventName) {
                ...
                print("Event Properties are \(eventProperties)")
                ...
                var eventModelList: [EventRequestModel.Event] = [eventModel]
                print("CustomerID in SendEvent \(getCustomerId())")
                var eventRequestModel = EventRequestModel.EventRequest(
                    customerId: getCustomerId(),
                    deviceId: getUniqueIdentifier(),
                    events: eventModelList
...
//                                }
    ...
    public func sendAttribute(attributeName: String, attributeValue: String) {
            do {
                ...
                print("Attribute value in sendAttribute \(attributeValue)")

                if attributeMap.keys.contains(attributeName) {
                    self.lastAttributeName = attributeName
                    ...
                    if(self.getAttributeValue(attributeId: String(attributeRequestModel.attributeId)) != attributeValue){
                        ...
                        DispatchQueue.global().asyncAfter(deadline: .now() + 3.0) {
                            ...
                            do {
                                //Generating again because now it is new customer ID
                                attributeRequestModel = AttributeRequestModel.AttributeRequest(
                                    customerId: self.getCustomerId(),
                                    attributeId: EventManager.shared.getAttribute(byName: attributeName)?.id ?? 0,
                                    value: attributeValue
                                )
                                   try self.postAttributeToServer(attributeRequestModel: attributeRequestModel)
                                print("Attribute value in sendAttribute 1 \(attributeRequestModel.value)")
                               } catch {
    ...
    func postAttributeToServer(attributeRequestModel: AttributeRequestModel.AttributeRequest) {
        do {
            ...
            print("Attribute value in postAttributeServer \(attributeRequestModel.value)")

            NetworkManager.shared.recordAttribute(requestBody: requestBody) { result in
                switch result {
                case .success(let response):
                    UserDefaults.standard.set(attributeRequestModel.value, forKey: String(attributeRequestModel.attributeId))
